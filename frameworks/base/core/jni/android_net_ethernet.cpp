...
namespace android {
    ...
    static jstring android_net_ethernet_waitForEvent(JNIEnv *env, jobject clazz)
    {
        ...
        LOGE("Poll events from ethernet devices");
        /*
        ...
        if ((len = recvmsg(nl_socket_poll, &msg, 0)) >= 0) {
            ...
            for (nh = (struct nlmsghdr *) buff; NLMSG_OK (nh, len);
                ...
                LOGE(" event :%d  found", nh->nlmsg_type);
                einfo = (struct ifinfomsg *)NLMSG_DATA(nh);
                LOGE("the device flag :%X", einfo->ifi_flags);
            ...
            LOGE("Done parsing");
            rbuf[4096 - left] = '\0';
            LOGE("poll state :%s, left:%d", rbuf, left);
        }
                                                         ...
                                                         jint index)
    {
        int i = 0;
        interface_info_t *info;
        LOGE("User ask for device name on %d, list:%X, total:%d",
             index, (unsigned int)interfaces, total_int);
        info = interfaces;
        if (total_int != 0 && index <= (total_int - 1)) {
            while (info) {
                if (index == i) {
                    LOGE("Found: %s", info->name);
