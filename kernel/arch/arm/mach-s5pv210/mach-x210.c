...
static struct resource smdkv210_dm9000_resources[] = {
	[0] = {
		...
		.end	= S5PV210_PA_SROM_BANK5 + 0x3,
		.flags	= IORESOURCE_MEM,
	},
	[1] = {
		.start	= S5PV210_PA_SROM_BANK5 + 0x4,
		.end	= S5PV210_PA_SROM_BANK5 + 0x7,
		.flags	= IORESOURCE_MEM,
	},
	[2] = {
		.start	= IRQ_EINT(10),
...
static int smdkv210_backlight_init(struct device *dev)
{
	...
	if (ret) {
		printk(KERN_ERR "failed to request GPD for-OUT 3\n");
		return ret;
	}
...
static void __init x210_dm9000_init(void) 
{ 
	unsigned int tmp; 
...
static struct s3c_hwmon_pdata x210_hwmon_pdata __initdata =
{
	/* Battery voltage (?-4.2V) */
	.in[0] = &(struct s3c_hwmon_chcfg) {
		.name = "x210:battery-voltage",
		.mult = 3300,
		.div = 2048,
	},
};
...
//	samsung_keypad_set_platdata(&smdkv210_keypad_data);
	...
	s3c_hwmon_set_platdata(&x210_hwmon_pdata);
