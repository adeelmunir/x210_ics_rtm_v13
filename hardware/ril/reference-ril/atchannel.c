...
static int writeline (const char *s)
{
    ...
    return 0; // i added this comment on drivers branch
}
static int writeCtrlZ (const char *s)
{
    size_t cur = 0;
...
void at_close()
{
    if (s_fd >= 0) {
        if(close(s_fd) != 0)
        LOGE("%s() FAILED to close fd %d!", __func__, s_fd);
    }
    s_fd = -1;
...
AT_CME_Error at_get_cme_error(const ATResponse *p_response)
{
    ...
    /**
     *  2014-1-26
     *  wythe: Delete for new error code defination.
     */
#if 0
