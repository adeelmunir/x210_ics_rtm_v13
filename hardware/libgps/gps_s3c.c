...
static int
s3c_channel_open_tty( GpsState*      state)
{
    ...
    ret = snprintf(state->device, sizeof state->device, "/dev/%s", prop);
...
static int
s3c_gps_init(GpsCallbacks* callbacks)
{
...
static int
s3c_gps_inject_time(GpsUtcTime time, int64_t timeReference, int uncertainty)
{
    return 0;
}

static int
s3c_gps_inject_location(double latitude, double longitude, float accuracy)
{
    return 0;
}

static void
s3c_gps_delete_aiding_data(GpsAidingData flags)
{
}

static int s3c_gps_set_position_mode(GpsPositionMode mode, int fix_frequency)
{
...
static const void*
s3c_gps_get_extension(const char* name)
{
    // no extensions supported
    return NULL;
}

static const GpsInterface  s3cGpsInterface = {
    sizeof(GpsInterface),
    s3c_gps_init,
    s3c_gps_start,
    s3c_gps_stop,
    s3c_gps_cleanup,
    s3c_gps_inject_time,
    s3c_gps_inject_location,
    s3c_gps_delete_aiding_data,
    s3c_gps_set_position_mode,
    s3c_gps_get_extension,
};

const GpsInterface* gps__get_gps_interface(struct gps_device_t* dev)
{
    return &s3cGpsInterface;
}
