...
static int hardware_flow_control = 0; /*0:open hardware flow control;1:close hardware flow control*/
...
static int cmux_N1 = 130;//1509 -> 130 [ length(1B) + data(<=127B) + FCS(1B) + FLAG(1B) ]
#if 0
	...
	if (channel->fd < 0) // is this channel free?
				{
                    ...
                    sprintf(command,"rm %s",link_name);
                    system(command);

                    if(symlink(pts,link_name) != 0)
                    {
                        LOGMUX(LOG_DEBUG,"Create link %s Error : %d",link_name,errno);
                        return 1;
                    }
...
//data from logical channel
				...
				if ((write_result = write(channellist[frame->channel].fd, frame->data, frame->length)) >= 0)
				{
                    ...
                    if(write_result < frame->length)
                    {
                        ...
                        LOGMUX(LOG_DEBUG,"Write pts error1\n\n\n$$$$$$$$$$$$$$$$$$$$$$$$n\n");
                        //usleep(1000*1000);
                    }
                }
				else{
                    ...
                    if(write_result < frame->length)
                    {
                        ...
                        //MSC_flag |=  1 << (frame->channel - 1);
                        LOGMUX(LOG_DEBUG,"Write pts error2\n\n\n$$$$$$$$$$$$$$$$$$$$$$$$n\n");
                        //usleep(1000*1000);
                    }
#if 0
					switch (errno){
...
/* open the serial port */
	SYSCHECK(serial->fd = open(serial->devicename, O_RDWR | O_NOCTTY));
...
//joe
    LOGMUX(LOG_INFO, "datacount: %d",serial.in_buf->datacount);
	sleep(5);
}
